...
png(filename="K:\\Pricing Product General\\Users\\Diana Rivera\\Projects\\Coursera\\plot4.png")
par(mfrow=c(2,2))
plot(elec.1$date.time, elec.1$Global_active_power, xlab="", ylab = 'Global Active Power', type="l")
elec.1$Voltage <- as.character(elec.1$Voltage)
elec.1$Voltage <- as.numeric(elec.1$Voltage)
elec.1$Global_reactive_power <- as.character(elec.1$Global_reactive_power)
elec.1$Global_reactive_power <- as.numeric(elec.1$Global_reactive_power)
plot(elec.1$date.time, elec.1$Voltage, type="l", xlab="datetime", ylab="Voltage")
plot(elec.1$date.time, elec.1$Sub_metering_1, col = "black", type = "l", xlab = "", ylab="Energy sub metering")
lines(elec.1$date.time, elec.1$Sub_metering_2, col = "red", type = "l")
lines(elec.1$date.time, elec.1$Sub_metering_3, col = "blue", type = "l")
legend("topright",  c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"), col=c('black','red','blue'), horiz=FALSE, lty=1, cex = 0.8, box.lty=0)
box()
plot(elec.1$date.time, elec.1$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
dev.off()